...
function GameCard({ game }: { game: Game }) {
  ...
  return (
    <div className={`bg-gray-900 rounded-xl border p-4 transition hover:border-gray-600 ${
      isLive ? 'border-green-500/40' : 'border-gray-800'
    }`}>
      {/* League + status */}
      ...
      <div className={`flex items-center justify-between py-1.5 ${awayWins ? 'opacity-100' : isFinal ? 'opacity-50' : ''}`}>
        <div className="flex items-center gap-2.5">
          ...
          <span className={`text-sm ${awayWins ? 'font-bold text-white' : 'text-gray-300'}`}>{away.abbr}</span>
        </div>
      ...
      <div className={`flex items-center justify-between py-1.5 ${homeWins ? 'opacity-100' : isFinal ? 'opacity-50' : ''}`}>
        <div className="flex items-center gap-2.5">
          ...
          <span className={`text-sm ${homeWins ? 'font-bold text-white' : 'text-gray-300'}`}>{home.abbr}</span>
        </div>
