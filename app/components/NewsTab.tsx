...
const CATEGORY_COLORS: Record<string, { badge: string; dot: string }> = {
  ...
  Business:      { badge: 'bg-purple-500/15 text-purple-400 border-purple-500/25', dot: 'bg-purple-400' },
...
function NewsCard({ item }: { item: NewsItem }) {
  const colors = CATEGORY_COLORS[item.category] ?? { badge: 'bg-gray-800 text-gray-400 border-gray-700', dot: 'bg-gray-400' };
  ...
  return (
    <a
      ...
      className="group flex flex-col gap-3 bg-gray-900 rounded-xl border border-gray-800 p-4 hover:border-gray-600 transition-colors"
    >
      {/* Category + time */}
      ...
      {/* Headline */}
      <p className="text-sm font-semibold text-gray-100 leading-snug group-hover:text-white transition-colors line-clamp-3">
        {item.title}
      </p>
              ...
              className={`px-3.5 py-1.5 rounded-lg text-xs font-bold font-mono tracking-wide transition border ${
                isActive
                  ? cat === 'All'
                    ...
                    : `${colors.badge}`
                  : 'bg-gray-900 text-gray-500 border-gray-800 hover:text-white hover:border-gray-600'
              }`}
            >
              {cat}
              {cat !== 'All' && !loading && (
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="bg-gray-900 rounded-xl border border-gray-800 p-4 animate-pulse h-40" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
