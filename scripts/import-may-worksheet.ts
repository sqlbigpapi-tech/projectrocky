...
const FILE = '/Users/davidortiz/Downloads/2026-05_Worksheet.xlsx';
const YEAR = 2026;
...
async function main() {
  ...
  console.log(`=== May 2026 Worksheet Import ${dryRun ? '(DRY RUN)' : ''} ===\n`);
  ...
  for (const market of MARKETS) {
    ...
    const may = 4;
    console.log(
      `${market}: ${actualMonths}mo actuals | May rev $${(actual.totalRevenue[may]/1000).toFixed(0)}K plan $${(plan.totalRevenue[may]/1000).toFixed(0)}K | ` +
      `May NI $${(actual.netIncome[may]/1000).toFixed(1)}K plan $${(plan.netIncome[may]/1000).toFixed(1)}K | ` +
  ...
  const { data: miaMay } = await db.from('pl_monthly')
    .select('revenue, sga, cons_labor, net_income, is_forecast')
    .eq('year', YEAR).eq('month', 5).eq('market', 'MIA').single();
  console.log('\nMIA May 2026 sample:', miaMay);
}
