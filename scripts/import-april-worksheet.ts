...
const FILE = '/Users/davidortiz/Downloads/2026-04_Worksheet.xlsx';
const YEAR = 2026;
...
async function main() {
  ...
  console.log(`=== April 2026 Worksheet Import ${dryRun ? '(DRY RUN)' : ''} ===\n`);
  ...
  for (const market of MARKETS) {
    ...
    const apr = 3;
    console.log(
      `${market}: ${actualMonths}mo actuals | Apr rev $${(actual.totalRevenue[apr]/1000).toFixed(0)}K plan $${(plan.totalRevenue[apr]/1000).toFixed(0)}K | ` +
      `Apr NI $${(actual.netIncome[apr]/1000).toFixed(1)}K plan $${(plan.netIncome[apr]/1000).toFixed(1)}K | ` +
  ...
  const { data: miaApr } = await db.from('pl_monthly')
    .select('revenue, sga, cons_labor, net_income, is_forecast')
    .eq('year', YEAR).eq('month', 4).eq('market', 'MIA').single();
  console.log('\nMIA Apr 2026 sample:', miaApr);
}
